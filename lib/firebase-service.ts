...
export async function downloadProposal(proposal: any): Promise<void> {
  try {
    // Generate proposal content as text
    const proposalContent = generateProposalText(proposal)
    ...
    link.href = url
    link.download = `proposal-${proposal.id || "unknown"}-${new Date().toISOString().split("T")[0]}.txt`
...
function generateProposalText(proposal: any): string {
...
Company: ${proposal.client?.company || "N/A"}
Client: ${proposal.client?.name || "N/A"}
Email: ${proposal.client?.email || "N/A"}
Date: ${createdDate}

PROPOSAL DETAILS
================
Total Amount: ₱${proposal.totalAmount?.toLocaleString("en-PH", { minimumFractionDigits: 2 }) || "0.00"}
...
Traffic Count: ${product.specs_rental?.traffic_count?.toLocaleString() || "N/A"} vehicles
Visibility: ${product.specs_rental?.elevation || "N/A"} meters
Dimensions: ${product.specs_rental?.height || "N/A"}ft (H) x ${product.specs_rental?.width || "N/A"}ft (W)
Type: ${product.type || "Advertising Space"}
`
