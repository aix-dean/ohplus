...
export async function generateReportPDFAsBase64(reportData: ReportData): Promise<string> {
  try {
    ...
    doc.text(`Site Name: ${reportData.siteName}`, 20, yPosition)
    yPosition += 10

    doc.text(`Report Type: ${reportData.reportType}`, 20, yPosition)
    yPosition += 10

    if (reportData.date) {
      doc.text(`Date: ${new Date(reportData.date).toLocaleDateString()}`, 20, yPosition)
      yPosition += 10
    }
    ...
    if (reportData.assignedTo) {
      doc.text(`Assigned To: ${reportData.assignedTo}`, 20, yPosition)
      yPosition += 10
    }
    ...
    if (reportData.description) {
      ...
      const splitText = doc.splitTextToSize(reportData.description, 170)
      doc.text(splitText, 20, yPosition)
      yPosition += splitText.length * 5
    }

    // Add completion percentage if available
    if (reportData.completionPercentage !== undefined) {
      yPosition += 10
      doc.text(`Completion: ${reportData.completionPercentage}%`, 20, yPosition)
    }
...
export async function generateReportPDFFromId(reportId: string): Promise<Blob> {
  try {
    ...
    const base64 = await generateReportPDFAsBase64(report)
...
export async function generateReportPDF(
  ...
  try {
    const base64 = await generateReportPDFAsBase64(reportData)
