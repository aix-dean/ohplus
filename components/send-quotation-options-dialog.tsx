...
export function SendQuotationOptionsDialog({
  ...
  const publicViewUrl = quotation?.id ? `https://ohplus.aix.ph/quotations/view/${quotation.id}` : ""
  ...
  const handleCopyLink = () => {
    if (publicViewUrl) {
      ...
      toast({
        title: "Error",
        description: "Quotation link is not available.",
        variant: "destructive",
      })
    }
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] max-h-[80vh] overflow-y-auto p-6">
        <DialogHeader className="mb-4">
          ...
          <DialogDescription className="sr-only">Choose how to send this quotation.</DialogDescription>
        </DialogHeader>
          ...
          {quotation && (
            <div className="flex items-center gap-4 mb-2">
              <div className="relative w-20 h-20 flex-shrink-0">
                <Image
                  src={quotation.image_url || "/placeholder.svg?height=80&width=80&text=No+Image"}
                  alt="Quotation Image"
                  width={80}
                  height={80}
              ...
              <div className="flex flex-col">
                <span className="text-lg font-semibold">{quotation.quotation_number || "Untitled Quotation"}</span>
