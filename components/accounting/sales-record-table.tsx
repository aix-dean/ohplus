...
function recompute(row: SalesRecordRow): SalesRecordRow {
  ...
  // Spec says Amount Collected = Invoice Amount – Creditable Tax.
  // We treat "Invoice Amount" as the computed Total for Sales Record.
  const amountCollected = total - creditableTax
  return {
    ...row,
    outputVat,
    total,
    // Discount is listed but no formula provided in the spec; keeping 0 as view-only.
    discount: 0,
    creditableTax,
    amountCollected,
  }
...
export function SalesRecordTable() {
  ...
  const filtered = useMemo(
    () => rows.filter((r) => includesAny(r, query)),
    [rows, query]
  )
  ...
  function updateRow(id: string, patch: Partial<SalesRecordRow>) {
    setRows((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r
        const next = { ...r, ...patch }
        // If Net Sales changed, recompute.
