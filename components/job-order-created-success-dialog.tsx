...
export function JobOrderCreatedSuccessDialog({ isOpen, onDismissAndNavigate }: JobOrderCreatedSuccessDialogProps) {
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onDismissAndNavigate}>
      <DialogContent className="sm:max-w-[425px] flex flex-col items-center justify-center text-center p-8">
        <h2 className="text-3xl font-bold mb-4">Job Order Created!</h2>
        <Image src="/party-popper.png" alt="Party Popper" width={120} height={120} className="mb-6" />
        <p className="text-lg text-gray-700">Your Job Order has been successfully created!</p>
      </DialogContent>
    </Dialog>
  )
}
