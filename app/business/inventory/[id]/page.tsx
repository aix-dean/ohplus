...
export default function BusinessProductDetailPage({ params }: Props) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // Filter out removed images and combine with new media (new uploads become the new thumbnail)
      const existingMedia = (product.media || []).filter(mediaItem => !imagesToRemove.includes(mediaItem.url))
      const allMedia = [...mediaUrls, ...existingMedia]
                          ...
                          >
                            {file.type.startsWith('video/') ? (
                              <div className="w-full h-full bg-black flex items-center justify-center relative">
                                ...
                                <div className="absolute bottom-0 right-0 bg-black/50 text-white text-xs px-1 py-0.5 rounded">
                                  V
                                </div>
                              </div>
                            ) : (
