...
export default function ComposeEmailPage() {
  ...
  const preGenerateAllPDFs = useCallback(
    async (mainQuotation: Quotation, relatedQuotations: Quotation[]) => {
      ...
      try {
        ...
        console.log("[DEBUG] uniqueRelatedQuotations in preGenerateAllPDFs:", uniqueRelatedQuotations.map(q => q.id));
        setPreGeneratedPDFs(allPDFs)
        console.log(`[DEBUG] All PDFs pre-generated successfully. Total: ${allPDFs.length}`)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const attachSingle = searchParams.get("attachSingle") === "true" // Removed declaration, now using top-level searchParams
      console.log("[DEBUG] attachSingle parameter:", attachSingle)

      let related: Quotation[] = []
      if (attachSingle) {
        console.log("[DEBUG] Logic: Attaching only single quotation:", quotation.id)
        related = [quotation]
        setRelatedQuotations([quotation])
        setPdfAttachments([
          `QT-${quotation.quotation_number}_${quotation.client_company_name || "Client"}_Quotation.pdf`,
        ])
      } else { // Only proceed with page_id logic if attachSingle is false
        if (quotation?.page_id) {
          console.log("[DEBUG] Logic: Attaching all related quotations for page_id:", quotation.page_id)
          ...
          console.log("[DEBUG] Logic: No page_id and not attachSingle, attaching single quotation:", quotation.id)
          related = [quotation]
          setRelatedQuotations([quotation])
          setPdfAttachments([
            `QT-${quotation.quotation_number}_${quotation.client_company_name || "Client"}_Quotation.pdf`,
...
${user?.email || ""}`)
  ...
  }, [params.id, user, userData, toast, preGenerateAllPDFs, searchParams])
  ...
  const handleAddTemplate = async () => {
    ...
    console.log("[DEBUG] handleAddTemplate - companyId:", companyId)
    ...
    try {
      ...
      console.log("[DEBUG] handleAddTemplate - templateData:", templateData)
      const newTemplate = await emailService.createEmailTemplate(templateData)
      console.log("[DEBUG] handleAddTemplate - newTemplate created:", newTemplate)
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const requestBody = {
        quotation: quotation,
        clientEmail: toEmail,
        client: { name: quotation.client_name, company: quotation.client_name, email: quotation.client_email },
  ...
  const handleSuccessDialogClose = () => {
    console.log("[DEBUG] handleSuccessDialogClose called. Redirecting to /sales/quotations");
