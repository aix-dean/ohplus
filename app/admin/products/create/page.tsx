...
export default function CreateProductPage() {
  ...
  const handleMediaUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(files)) {
        const storageRef = ref(storage, `product_media/${user?.uid}/${file.name}_${Date.now()}`)
  ...
  const handleRemoveMedia = (urlToRemove: string) => {
    ...
    toast({
      title: "Media Removed",
      description: "File removed from list. It will not be saved unless you submit.",
    })
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const newProductRef = doc(db, "products") // Firestore will auto-generate ID
      ...
      const newProduct: Product = {
        ...
        seller_id: user.uid,
        seller_name: userData?.display_name || userData?.email || "Unknown",
      }

      if (type === "rental") {
        newProduct.specs_rental = rentalSpecs
      } else if (type === "light") {
        newProduct.light = lightSpecs
      }
